...
public class CanvasPersistence {
    ...
    /**
     * Save canvas content as byte array - RGB ONLY (NO ALPHA)
     */
    public static byte[] saveCanvasToBytes(Canvas canvas) {
        try {
            ...
            // Use unscaled snapshot for DrawingCanvas to get maximum quality
            WritableImage snapshot;
            if (canvas instanceof DrawingCanvas) {
                snapshot = ((DrawingCanvas) canvas).getUnscaledSnapshot();
            } else {
                snapshot = canvas.snapshot(null, null);
            }

            // Convert to BufferedImage using TYPE_INT_RGB (NO ALPHA CHANNEL)
            BufferedImage rgbImage = new BufferedImage(
                    (int)snapshot.getWidth(),
                    (int)snapshot.getHeight(),
                    BufferedImage.TYPE_INT_RGB  // CRITICAL: NO ALPHA CHANNEL
            );

            // Create graphics context for RGB conversion
            ...
            // Draw the snapshot onto RGB image (alpha gets flattened to white)
            BufferedImage fxImage = SwingFXUtils.fromFXImage(snapshot, null);
            g2d.drawImage(fxImage, 0, 0, null);
            g2d.dispose();

            // Convert to PNG bytes
            ByteArrayOutputStream baos = new ByteArrayOutputStream();
            boolean success = ImageIO.write(rgbImage, "PNG", baos);

            if (success) {
                byte[] result = baos.toByteArray();
                System.out.println("Canvas saved (RGB-only): " + result.length + " bytes");
                return result;
            } else {
    ...
    /**
     * Restore canvas content from byte array - REPLACE MODE (NO BLENDING)
     */
    public static boolean restoreCanvasFromBytes(Canvas canvas, byte[] imageData) {
        try {
            ...
            // CRITICAL: Clear canvas and set opaque white background
            gc.clearRect(0, 0, canvas.getWidth(), canvas.getHeight());
            gc.setFill(Color.WHITE);
            gc.fillRect(0, 0, canvas.getWidth(), canvas.getHeight());

            // CRITICAL FIX: Set graphics context to replace mode (no alpha blending)
            gc.save();

            // Disable image smoothing
            gc.setImageSmoothing(false);

            // Set global alpha to 1.0 (fully opaque)
            gc.setGlobalAlpha(1.0);

            // CRITICAL: Use source-over composite mode with no alpha blending
            gc.drawImage(image, 0, 0, canvas.getWidth(), canvas.getHeight());

            gc.restore();

            System.out.println("Canvas restored (no alpha blending): " + imageData.length + " bytes");
            return true;
